(function() {
  ...
  function initRobot() {
    ...
    columnPositions.forEach(([x, z]) => {
      const columnGeo = new THREE.BoxGeometry(0.4, 7, 0.4, 2, 12, 2);
      const column = new THREE.Mesh(columnGeo, factoryMaterial);
      column.position.set(x, 3.5, z);
      scene.add(column);

      const capGeo = new THREE.BoxGeometry(0.6, 0.3, 0.6, 2, 1, 2);
      const cap = new THREE.Mesh(capGeo, factoryMaterial);
      cap.position.set(x, 7.15, z);
      scene.add(cap);
    });

    const beamPositions = [
      {start: [-6, 7, -5], end: [-6, 7, 5]},
      {start: [6, 7, -5], end: [6, 7, 5]},
      {start: [-6, 7, -5], end: [6, 7, -5]},
      {start: [-6, 7, 0], end: [6, 7, 0]},
      {start: [-6, 7, 5], end: [6, 7, 5]},
      {start: [0, 7, -8], end: [0, 7, 8]}
    ];
    ...
    beamPositions.forEach(({start, end}) => {
      ...
      const beamGeo = new THREE.BoxGeometry(0.3, 0.3, length, 1, 1, 8);
      const beam = new THREE.Mesh(beamGeo, factoryMaterial);
      const midX = (start[0] + end[0]) / 2;
      const midZ = (start[2] + end[2]) / 2;
      beam.position.set(midX, start[1], midZ);
      if (start[0] !== end[0]) {
        beam.rotation.y = Math.atan2(end[0] - start[0], end[2] - start[2]);
      }
      scene.add(beam);
    });

    const pipeConfigs = [
      {start: [-6, 5.5, -5], end: [-6, 5.5, 5], radius: 0.12},
      {start: [6, 5.5, -5], end: [6, 5.5, 5], radius: 0.12},
      {start: [-6, 4.5, -5], end: [6, 4.5, -5], radius: 0.1},
      {start: [-6, 4.5, 5], end: [6, 4.5, 5], radius: 0.1},
      {start: [-5, 6, -7], end: [5, 6, -7], radius: 0.15},
      {start: [-5, 5, -7], end: [5, 5, -7], radius: 0.08}
    ];

    pipeConfigs.forEach(({start, end, radius}) => {
      const length = Math.sqrt(
        Math.pow(end[0] - start[0], 2) +
        Math.pow(end[1] - start[1], 2) +
        Math.pow(end[2] - start[2], 2)
      );
      const pipeGeo = new THREE.CylinderGeometry(radius, radius, length, 6, 4);
      const pipe = new THREE.Mesh(pipeGeo, factoryMaterial);
      const midX = (start[0] + end[0]) / 2;
    ...
    const machineryConfigs = [
      {pos: [-4, 0.5, -6], size: [1.2, 1, 1.5]},
      {pos: [4, 0.6, -6], size: [1.5, 1.2, 1.2]},
      {pos: [-7, 0.4, 2], size: [0.8, 0.8, 1]},
      {pos: [7, 0.5, 2], size: [1, 1, 1.3]}
    ];

    machineryConfigs.forEach(({pos, size}) => {
      const machineGeo = new THREE.BoxGeometry(size[0], size[1], size[2], 3, 3, 3);
      const machine = new THREE.Mesh(machineGeo, factoryMaterial);
      machine.position.set(pos[0], pos[1], pos[2]);
      scene.add(machine);

      const detailGeo = new THREE.CylinderGeometry(0.15, 0.15, 0.3, 6);
      const detail = new THREE.Mesh(detailGeo, factoryMaterial);
      detail.position.set(pos[0], pos[1] + size[1]/2 + 0.15, pos[2]);
      scene.add(detail);
    });

    const ceilingTrusses = [
      {pos: [-3, 6.8, -3], rot: 0},
      {pos: [3, 6.8, -3], rot: 0},
      {pos: [-3, 6.8, 3], rot: 0},
      {pos: [3, 6.8, 3], rot: 0}
    ...
    ceilingTrusses.forEach(({pos, rot}) => {
      const trussGeo = new THREE.BoxGeometry(2, 0.15, 0.15, 8, 1, 1);
      const truss = new THREE.Mesh(trussGeo, factoryMaterial);
      truss.position.set(pos[0], pos[1], pos[2]);
